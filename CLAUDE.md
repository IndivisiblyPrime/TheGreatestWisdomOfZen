...
/ (Homepage)
  └── BookHero ('use client')
        ...
        — Preloads `/acquire` on mount (route via `router.prefetch('/acquire')`, plus its `backgroundImage`/`backgroundImageMobile`/`brushStrokeImage` via `new Image()`) — see "No-flash video → /acquire handoff" below
