...
/ (Homepage)
  └── BookHero ('use client')
        ...
        — Preloads `/acquire` on mount (route via `router.prefetch('/acquire')`, plus its `backgroundImage`/`backgroundImageMobile`/`brushStrokeImage` via `react-dom`'s `preload()`) — see "No-flash video → /acquire handoff" below
        — Won't start playback until the active video has buffered its full duration (polls `buffered`/`readyState`, not the `canplaythrough` event) — see "Video-readiness gate" below
...
- **No-flash video → /acquire handoff (`BookHero`)**: a `useEffect` on mount calls `router.prefetch('/acquire')` and preloads `/acquire`'s `backgroundImage`/`backgroundImageMobile`/`brushStrokeImage` via `react-dom`'s `preload(url, { as: 'image' })` (same `urlFor(...).width(...)` values `AcquireSection` renders, so the URLs — and therefore the cache entries — match exactly). Runs unconditionally (a cached image/prefetch is a no-op refetch), so it also warms things when there's no video. Without this, the click → video → `/acquire` path had a real gap: the click handler is a plain `onClick`, not a `<Link>`, so Next's automatic viewport prefetching never touched `/acquire`; and `/acquire`'s background photo was — by design, per the "No-flash background/video layering" fix above — never rendered anywhere on the homepage while a video is configured, so the browser had never fetched it. Either gap meant `/acquire` briefly showed the page's white body background (`globals.css` `--background`) after the video ended and before the photo/route arrived — the reported "flash of white." Preloading during video playback (which runs a couple seconds, plenty of time on a normal connection) means both are already cached by the time `onEnded` fires. **Use `preload()`, not `new Image()`:** the first version of this fix used `new window.Image(); img.src = url` with no retained reference — that object is eligible for garbage collection before the fetch completes, which silently cancels the download in some browsers (Safari/iOS is the most aggressive about this), especially likely here since the image is kicked off on mount and the user may sit on the homepage for several seconds before clicking. That made the fix unreliable in exactly the real-world conditions it was meant to help, which is why the flash was reported as still present (worse, even, since the now-added `router.prefetch` call competes for bandwidth with the video download for no payoff if the image preload silently failed). `preload()` inserts a real `<link rel="preload">` that React/the browser own for the page's lifetime, so it can't be GC'd out from under the fetch. Verified in a production build (`next start`, prefetching is a no-op in `next dev`) under both fast and throttled/cold-cache network conditions (`Network.emulateNetworkConditions` + `Network.clearBrowserCache` via CDP): captured the actual click → video → `/acquire` transition frame-by-frame via `Page.startScreencast` and checked every frame's average brightness — it never approaches white at any point in the sequence, including during the crossfade and through the full ~3.3s card/nav entrance animation.
- **Video-readiness gate (`BookHero`)**: clicking is a no-op (`handleClick` returns early) until the active video has buffered its full duration, tracked by a `useEffect` that polls `video.buffered`/`readyState` on a 300ms interval rather than listening for the `canplaythrough` event. The "Click to continue" prompt's 5s timer is also gated on this, so it never invites a click that would do nothing. **Why polling instead of `canplaythrough`:** verified directly (via CDP network throttling) that a video can sit at `readyState === 4` with `buffered` covering its *entire* duration — i.e. fully downloaded — while the `canplaythrough` event never fires at all; gating exclusively on that event would have permanently blocked clicking in that case. Polling the underlying buffered range converges regardless of which lifecycle events the browser does or doesn't fire. Without this gate, clicking before the video has enough buffered starts playback that immediately stalls waiting on more data — confirmed directly: under a throttled connection, a video that had buffered barely 0.6s of a 5s clip took **30+ seconds** of real time to actually finish playing once started early, repeatedly freezing mid-scene.
