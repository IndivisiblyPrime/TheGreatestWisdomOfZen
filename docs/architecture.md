# Architecture — The Greatest Wisdom of Zen

The living technical record for this repo: structure, design system, page-by-page
behaviour, schema, and the hard-won gotchas behind them. `CLAUDE.md` is the short
orientation; this is the detail it points at.
...
**The page canvas is `#e7ceb2`, not white.** `body` in `globals.css` sets this explicitly rather than
using the white `bg-background` token. It is the background photograph's own average tone, so any
moment where the canvas is visible — an image still rasterising, a route swap, a compositing seam —
reads as part of the photo instead of a white flash. Don't revert it to `bg-background`; see
"No white flash at the video → /acquire cut" for the full history.
...
`globals.css` defaults `--app-h` to `100dvh` (with a `100vh` `@supports` fallback). **`dvh` is authoritative; `src/components/ViewportSync.tsx` only intervenes when the engine is measurably wrong.** It renders a hidden `100dvh` probe and, on a debounced settle, removes any override, lets `dvh` re-resolve, and re-applies a pixel value only if the probe still disagrees with `window.innerHeight`. In normal operation nothing is overridden at all. The `removeProperty` doubles as the relayout trigger iOS skips on a page with no scrollable document, which was the original diagnosis and still holds.

**That conditional design is a correction of the first attempt, and the reasoning matters.** The original `ViewportSync` (2026-09-08) unconditionally wrote `innerHeight` into `--app-h` on every viewport event, which made the bug *more* likely rather than less: `100dvh` is recomputed continuously by the browser and is correct by construction at every instant, whereas a pixel snapshot is correct only until the next change. iOS fires `resize` repeatedly *through* its toolbar animation, so the last event could carry an intermediate height — and once written as an inline style on `<html>`, that value was latched permanently and `dvh` could no longer rescue it. The strip came back on jackharvey.me and the same flaw was live here (Jack, 2026-09-09). **Never write an unconditional snapshot over a live CSS value.**
...
Unlike jackharvey.me — whose phone view was restructured to plain document flow so it cannot exhibit the bug at all — this site's homepage genuinely needs a viewport-sized shell (a full-screen video that is also the click target), so it depends on the mechanism above. **Headless engines cannot reproduce real iOS toolbar-animation timing**, so a green Playwright run proves the steady states are correct, not the animation; verify on a device.
...
/ (Homepage)
  └── BookHero ('use client')
        ...
        — seamless transition: background image (when shown at all) on homepage matches /acquire on desktop, so video → /acquire is smooth
        ...
        — Mobile assets: both desktop and mobile `<video>` elements are always mounted; Tailwind `hidden md:block` / `md:hidden` classes pick which is shown, so it updates instantly on resize. Both videos are downloaded up front (accepted bandwidth tradeoff — avoids a JS-driven single-mount that wouldn't react to a mid-session resize). Click handler picks the active `<video>` ref via `window.matchMedia('(min-width: 768px)')` at click time, matching the CSS breakpoint. The homepage's mobile image is `backgroundImageMobile || backgroundImage`, same as every other page — there is deliberately no homepage-only "starting image" field; the opening experience is always the transition video, not a distinct static image.
        — Preloads `/acquire` on mount (route via `router.prefetch('/acquire')`, plus its `backgroundImage`/`backgroundImageMobile`/`brushStrokeImage` via `react-dom`'s `preload()` **and** `.decode()`) — see "No white flash at the video → /acquire cut" below
        — On video end, paints /acquire's background over the still-mounted video, then navigates on the next frame — same section
        — Won't start playback until the on-screen video has buffered its full duration (checked live off the element at click time, not from React state) — see "Video-readiness gate" below
...
- **History was rewritten on 2026-09-09** (`git filter-repo --replace-text`) to scrub the old value from all 63 commits; the default branch is clean. **But GitHub still serves the pre-rewrite commits by direct SHA** — a force-push does not delete unreachable objects, and there is no self-service way to make GitHub garbage-collect them. Only a request to GitHub Support purges them. This is harmless *because the secret was rotated first*, which is the actual mitigation; the rewrite is hygiene on top of it. If a live credential is ever committed here, treat rotation as the fix and the rewrite as optional.
- This repo is **public**, and GitHub secret-scanning push protection is enabled on it — but that only blocks *recognised provider formats* (AWS, Stripe, GitHub tokens). A homemade hex string like this one is not a known pattern and passed straight through, which is exactly how it got committed. Push protection is not a substitute for keeping secrets out of tracked files.
...
- **View transitions**: deliberately NOT used. The homepage → /acquire navigation is a plain `router.push` hard cut, and `globals.css` neutralises the UA's default root cross-fade should anything else ever start one. See "No white flash at the video → /acquire cut" below for why.
...
- **No-flash background/video layering (`BookHero`)**: the fallback `backgroundImage`/`mobileImage` `<img>` is only rendered when there's no transition video for that breakpoint (`!transitionVideoUrl` / `!mobileVideoUrl`) — it is never mounted underneath a video. A `<video>` element paints nothing until its first frame has decoded, but an `<img>` paints as soon as it's mounted; rendering the photo underneath the video unconditionally (as a "loading fallback") meant the photo painted first and was briefly visible before the video's first frame appeared — most noticeable on mobile, where decode is slower. A permanent `bg-black` backdrop sits behind both, so on the rare occasion a video hasn't decoded a frame yet, the gap reads as an imperceptible black instant rather than a flash of the photo. Fixed because the background photo was never supposed to be visible at all when a video is configured — only the video's own first frame.
- **No white flash at the video → /acquire cut (`BookHero`, `globals.css`)** — the design is that the transition video's final frame *is* `/acquire`'s background image (confirmed: video 1916×1080 vs image 1672×941, and mobile 1080×1920 vs 941×1672 — aspect ratios match to within 0.15%, so `object-cover` crops them identically). The cut should therefore be imperceptible, but a brief white flash kept appearing at exactly that moment, on both macOS and iOS Safari. It took three attempts; the first two were wrong, and it is worth recording why so they aren't retried.

  **What actually shows as white:** the page canvas. `body` used the shadcn `bg-background` token (`oklch(1 0 0)` = pure white) and body's background propagates to the viewport canvas, which is painted beneath everything. Against the warm sand photograph (`#e5cdb3`) that is a huge jump — the blue channel alone goes 179 → 255. So *any* momentary gap anywhere in this flow — a layer being torn down, a bitmap not yet rasterised, a compositing seam — surfaced as a white stab rather than something unnoticeable.

  **The fix is four layers, in order of importance:**
  1. **Warm canvas.** `body` is now an explicit `#e7ceb2` — the photo's own average tone, sampled from the two background images — instead of the white token. This is the layer that matters most, because it doesn't depend on correctly identifying the mechanism: it converts *any* residual gap from a white flash into a tone within ~2/255 of the photo already on screen. `--background` itself stays white for the shadcn components that reference it.
  2. **No view transition.** `navigateToAcquire` is a plain `router.push`. It used to be wrapped in `document.startViewTransition`, which was useless here (a cross-fade between two identical frames can only ever be a no-op or an artifact) and actively risky (the UA's default root cross-fade fades both snapshots simultaneously; whether that dips toward the canvas colour depends on the engine applying `mix-blend-mode: plus-lighter` inside an isolated image-pair). It was also mis-wired: App Router's `router.push()` returns `undefined`, not a promise, so `startViewTransition` captured its "new" snapshot *before* React had rendered `/acquire` — animating old→old while the real swap happened afterwards, unprotected. `globals.css` also neutralises `::view-transition-old/new(root)` as a backstop.
  3. **Destination painted before the swap.** On `ended`, `BookHero` renders `/acquire`'s exact background image over the still-mounted video, waits for a painted frame, then navigates. So the correct pixels are already on screen in an ordinary `<img>` before the `<video>` element (and its dedicated compositing layer — on iOS often a hardware overlay plane) is destroyed. The failure mode is benign: if the overlay hasn't painted, the video's identical last frame is still underneath it.
  4. **Bitmaps decoded up front.** Fetching bytes and decoding them are separate steps, and an `<img>` whose bitmap isn't ready paints *nothing*. So the preload effect also calls `.decode()` on each destination image and retains the elements in a ref, meaning the overlay in (3) can paint on the very next frame.

  **Two earlier attempts that did not fix it, and why:** (a) `router.prefetch('/acquire')` + `new window.Image(); img.src = url` — the image object had no retained reference, so it is eligible for garbage collection mid-download, which silently cancels the fetch in some browsers; and in any case fetching was never the bottleneck. (b) Switching that to `react-dom`'s `preload()` — correct in itself (it inserts a real `<link rel="preload">` that can't be GC'd, and it is still in the code) but it only guarantees *bytes*, not decode, layer lifetime, or the canvas colour underneath.

  **On verification — read this before "confirming" any future change here.** The first two attempts were both declared verified and both were wrong, for two separate reasons. First, the metric was too coarse: average frame brightness with a `> 220` threshold, when the photo itself averages 204–215 and the video's own frames range 156–215, so a real wash sat inside the noise. Measure the **blue channel** against the steady-state median instead — that is where white separates from warm sand. Second, and more important, **testing only in Chrome cannot show this class of bug.** Chrome applies `mix-blend-mode: plus-lighter` to the view-transition snapshots (verified via `getComputedStyle(document.documentElement, '::view-transition-old(root)')`), so it is structurally incapable of exhibiting the cross-fade dip. The current fix was checked in both Chromium and **WebKit** (`npx playwright install webkit`), driving the real click → video → `/acquire` flow and capturing every composited frame around the swap: no frame exceeds +12 blue over baseline in either engine, and the trace steps directly from the video's last frame `(233.9, 212.3, 185.8)` to `/acquire`'s background `(229.0, 205.3, 179.5)` with nothing in between. Note also that headless engines never reproduced the flash even with the *old* code, so a clean headless run is necessary but not sufficient — the hardware video-overlay teardown in (3) is real-device-only, which is why the fix is layered rather than resting on one mechanism.
- **Video-readiness gate (`BookHero`)**: clicking is a no-op (`handleClick` returns early) until the on-screen video has buffered its full duration. Without this, clicking before enough is buffered starts playback that immediately stalls waiting on more data — confirmed directly: under a throttled connection, a video that had buffered barely 0.6s of a 5s clip took **30+ seconds** of real time to finish playing once started early, repeatedly freezing mid-scene.
  - **`handleClick` reads the `<video>` element directly, never React state.** The polled `activeVideoReady` state is refreshed on a 300ms interval and exists only to gate the "Click to continue" prompt (being a poll stale there is harmless). An earlier version gated the *click* on that state too, which silently dropped any click landing in the window after buffering completed but before the next poll — the visitor clicks and nothing happens. Caught in testing as a run where playback never started (`playbackRate` still 1, no `play` event) despite the video being fully buffered.
  - **Only the active breakpoint's video is checked** — the off-breakpoint one is never seen, so waiting on it would block e.g. a desktop visitor on the mobile video's download.
  - **`play()` must stay inside the click handler.** Queueing an early click to fire once buffering finishes would move `play()` out of the user-gesture context and risk being blocked by autoplay policy on iOS, so an early click is ignored rather than deferred.
  - **Why `buffered`/`readyState` polling instead of the `canplaythrough` event:** verified directly (via CDP network throttling) that a video can sit at `readyState === 4` with `buffered` covering its *entire* duration while `canplaythrough` never fires at all — gating on that event alone would leave the page permanently unclickable.
  - **`navigateToAcquire` pairs its `requestAnimationFrame` with a `setTimeout` fallback**, because rAF is throttled or suspended entirely in a backgrounded tab; without it, switching tabs as the video ends could strand someone on the homepage forever. Whichever fires first wins, guarded by a `navigated` flag so only one push happens.
...
Homepage (`page.tsx`) — includes `brushStrokeImage` (not rendered on the homepage itself) so `BookHero` can preload it for the `/acquire` handoff, see "No-flash video → /acquire handoff" above:
```groq
